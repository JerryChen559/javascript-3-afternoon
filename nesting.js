...
let removeDuplicates = () => {
  return workplaceAccidents.filter((num, i, arr) => arr.indexOf(num) == i);
};
...
//Code Here
let recordCleaner = () => {
  myCar.accidents.map((val, i, arr) => {
    return (val.atFaultForAccident = false);
  });
};
...
let looper = arr => {
  for (let i = 0; i < numsArr.length; i++) {
    numsArr[i].map((val, ind, a) => {
      val % 2 === 0 ? (numsArr[i][ind] = "even") : (numsArr[i][ind] = "odd");
    });
  }
